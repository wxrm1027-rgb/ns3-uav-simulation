...
void
AnimationInterface::AddHiddenNode(uint32_t nodeId)
{
    m_hiddenNodeIds.insert(nodeId);
}

void
AnimationInterface::AddCustomLink(uint32_t fromId, uint32_t toId, const std::string& description)
{
    CustomLinkElement elt;
    elt.fromId = fromId;
    elt.toId = toId;
    ...
    elt.widthPx = 0;
    elt.dashed = false;
    m_customLinks.push_back(elt);
}

void
AnimationInterface::AddCustomLink(uint32_t fromId,
                                  uint32_t toId,
...
void
AnimationInterface::SetLinkStyle(uint32_t fromId,
                                 uint32_t toId,
                                 uint8_t colorR,
                                 uint8_t colorG,
                                 uint8_t colorB,
                                 ...
                                 bool dashed)
{
    P2pLinkNodeIdPair key;
    key.fromNode = fromId;
    key.toNode = toId;
    LinkStyleOverride ov;
    ov.r = colorR;
    ov.g = colorG;
    ov.b = colorB;
    ov.widthPx = widthPx;
    ov.dashed = dashed;
    m_linkStyleOverrides[key] = ov;
}
...
AnimationInterface::StartAnimation(bool restart)
{
    m_currentPktCount = 0;
    m_started = true;
    // Do not open file here: defer to DoInitialWrite so AddHiddenNode/AddCustomLink take effect first.
    // File will be opened and <anim> written at t=0.
    Simulator::Schedule(Seconds(0), &AnimationInterface::DoInitialWrite, this);
    if (!restart)
    {
        ConnectCallbacks();
    }
...
void
AnimationInterface::DoInitialWrite()
{
...
AnimationInterface::WriteLinkProperties()
{
        ...
        for (uint32_t i = 0; i < nDev; ++i)
        {
            ...
            { // Since these are duplex links, we only need to dump
                ...
                for (std::size_t j = 0; j < nChDev; ++j)
                {
                    ...
                    if (n1Id < n2Id)
                    {
                        if (m_hiddenNodeIds.count(n1Id) || m_hiddenNodeIds.count(n2Id))
                        {
                            continue; // skip links involving hidden nodes (e.g. EPC)
                        }
                        std::vector<std::string> ipv4Addresses = GetIpv4Addresses(dev);
                        ...
                        {
                            LinkProperties lp;
                            lp.fromNodeDescription = GetIpv4Address(dev) + "~" + GetMacAddress(dev);
                            lp.toNodeDescription = GetIpv4Address(chDev) + "~" + GetMacAddress(chDev);
                            lp.linkDescription = "";
                            lp.linkColorR = -1;
                            lp.linkColorG = -1;
                            lp.linkColorB = -1;
                            lp.linkWidth = 0;
                            lp.linkDashed = false;
                            LinkStyleOverrideMap::const_iterator oit = m_linkStyleOverrides.find(p2pPair);
                            if (oit == m_linkStyleOverrides.end())
                            {
                                P2pLinkNodeIdPair rev = {n2Id, n1Id};
                                oit = m_linkStyleOverrides.find(rev);
                            }
                            if (oit != m_linkStyleOverrides.end())
                            {
                                lp.linkColorR = oit->second.r;
                                lp.linkColorG = oit->second.g;
                                lp.linkColorB = oit->second.b;
                                lp.linkWidth = oit->second.widthPx;
                                lp.linkDashed = oit->second.dashed;
                            }
                        ...
                        {
                            LinkProperties lp;
                            lp.fromNodeDescription = GetIpv6Address(dev) + "~" + GetMacAddress(dev);
                            lp.toNodeDescription = GetIpv6Address(chDev) + "~" + GetMacAddress(chDev);
                            lp.linkDescription = "";
                            lp.linkColorR = -1;
                            lp.linkColorG = -1;
                            lp.linkColorB = -1;
                            lp.linkWidth = 0;
                            lp.linkDashed = false;
                            LinkStyleOverrideMap::const_iterator oit2 = m_linkStyleOverrides.find(p2pPair);
                            if (oit2 == m_linkStyleOverrides.end())
                            {
                                P2pLinkNodeIdPair rev2 = {n2Id, n1Id};
                                oit2 = m_linkStyleOverrides.find(rev2);
                            }
                            if (oit2 != m_linkStyleOverrides.end())
                            {
                                lp.linkColorR = oit2->second.r;
                                lp.linkColorG = oit2->second.g;
                                lp.linkColorB = oit2->second.b;
                                lp.linkWidth = oit2->second.widthPx;
                                lp.linkDashed = oit2->second.dashed;
                            }
                            m_linkProperties[p2pPair] = lp;
                        }
    ...
    // Write logical/custom links (e.g. node-to-SPN data channels) that are not P2P in the topology
    for (std::vector<CustomLinkElement>::const_iterator it = m_customLinks.begin();
         it != m_customLinks.end();
         ++it)
    {
        uint32_t fromId = it->fromId;
        uint32_t toId = it->toId;
        if (m_hiddenNodeIds.count(fromId) || m_hiddenNodeIds.count(toId))
        {
            continue;
        }
        P2pLinkNodeIdPair p2pPair;
        p2pPair.fromNode = fromId;
        p2pPair.toNode = toId;
        LinkProperties lp;
...
AnimationInterface::WriteNodes()
{
    for (NodeList::Iterator i = NodeList::Begin(); i != NodeList::End(); ++i)
    {
        ...
        if (m_hiddenNodeIds.count(nodeId))
        {
            // 占位节点：放在 (0,0) 且 size=0 不可见，避免极端坐标撑大视图导致业务节点看不见
            WriteXmlNode(nodeId, n->GetSystemId(), 0, 0);
            continue;
        }
        NS_LOG_INFO("Update Position for Node: " << nodeId);
        ...
        WriteXmlNode(nodeId, n->GetSystemId(), v.x, v.y);
    }
}
...
AnimationInterface::WriteNodeColors()
{
    ...
    {
        Ptr<Node> n = *i;
        if (m_hiddenNodeIds.count(n->GetId()))
        {
            continue;
        }
        Rgb rgb = {255, 0, 0};
...
AnimationInterface::WriteNodeSizes()
{
    for (NodeList::Iterator i = NodeList::Begin(); i != NodeList::End(); ++i)
    {
        ...
        if (m_hiddenNodeIds.count(nodeId))
        {
            // 隐藏节点仍写入 size 0，保证 XML 中所有 nodeId 均有 size，NetAnim 设置图标大小时不越界
            AnimationInterface::NodeSize s = {0, 0};
            m_nodeSizes[nodeId] = s;
            UpdateNodeSize(nodeId, 0, 0);
            continue;
        }
        NS_LOG_INFO("Update Size for Node: " << nodeId);
        AnimationInterface::NodeSize s = {1, 1};
        m_nodeSizes[nodeId] = s;
        UpdateNodeSize(nodeId, s.width, s.height);
    }
}
...
{
    if (std::isnan(locX) || std::isinf(locX))
    {
        locX = 0;
    }
    if (std::isnan(locY) || std::isinf(locY))
    {
        locY = 0;
    }
...
AnimationInterface::WriteXmlLink(uint32_t fromId, uint32_t toLp, uint32_t toId)
{
    ...
    {
        element.AddAttribute("lr", static_cast<uint32_t>(lprop.linkColorR));
        element.AddAttribute("lg", static_cast<uint32_t>(lprop.linkColorG));
        element.AddAttribute("lb", static_cast<uint32_t>(lprop.linkColorB));
    }
    ...
    if (lprop.linkDashed)
    {
        element.AddAttribute("dashed", static_cast<uint32_t>(1));
    }
    WriteN(element.ToString(), m_f);
}
...
{
    if (std::isnan(width) || std::isinf(width) || width < 0)
    {
        width = 1;
    }
    if (std::isnan(height) || std::isinf(height) || height < 0)
    {
        height = 1;
    }
    AnimXmlElement element("nu");
