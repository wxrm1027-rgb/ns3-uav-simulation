...
class AnimationInterface
{
  public:
    ...
    /**
     * \brief Hide a node from the animation (no node element, no links involving it).
     * \param nodeId Node ID to hide (e.g. EPC nodes 1,2,3). Call before Simulator::Run().
     */
    void AddHiddenNode(uint32_t nodeId);

    /**
     * \brief Add a logical link for visualization (e.g. node-to-SPN data channel).
     * \param fromId From node ID
     * \param toId To node ID
     * \param description Link description string. Call before Simulator::Run().
     */
    void AddCustomLink(uint32_t fromId, uint32_t toId, const std::string& description);

    /**
     * \brief Add a logical link with optional style (color, width, dashed) for visualization.
     *        Style is written as XML attributes lr, lg, lb, lw, dashed for viewers that support them.
     * \param fromId From node ID
     * \param toId To node ID
     * \param description Link description string
     * \param colorR Red 0-255, or -1 to omit
     * \param colorG Green 0-255, or -1 to omit
     * \param colorB Blue 0-255, or -1 to omit
     * \param widthPx Line width in pixels, 0 = default
     * \param dashed True for dashed line
     */
    void AddCustomLink(uint32_t fromId,
                       uint32_t toId,
    ...
    /**
     * \brief Set style for a P2P or logical link (by fromId, toId). Written as lr, lg, lb, lw, dashed in XML.
     *        Call before Simulator::Run(). Use for backhaul links (e.g. GMC–SPN) that are discovered as P2P.
     * \param fromId From node ID
     * \param toId To node ID
     * \param colorR Red 0-255
     * \param colorG Green 0-255
     * \param colorB Blue 0-255
     * \param widthPx Line width in pixels
     * \param dashed True for dashed line
     */
    void SetLinkStyle(uint32_t fromId,
                      uint32_t toId,
                      uint8_t colorR,
                      uint8_t colorG,
                      uint8_t colorB,
  ...
  private:
    ...
    struct LinkProperties
    {
        ...
        /// Optional link style (for XML lr, lg, lb, lw, dashed). -1 = unset, 0 width = default.
        int linkColorR;
        int linkColorG;
        int linkColorB;
        double linkWidth;
    ...
    /// Custom link element (logical link for visualization)
    ...
    std::set<uint32_t> m_hiddenNodeIds;              ///< node IDs to hide from animation
    std::vector<CustomLinkElement> m_customLinks;    ///< logical links to add (e.g. node-SPN)
    /// Optional style override for links (P2P or custom) by (fromId, toId). Applied when writing XML.
    ...
    typedef std::map<P2pLinkNodeIdPair, LinkStyleOverride, LinkPairCompare> LinkStyleOverrideMap;
    LinkStyleOverrideMap m_linkStyleOverrides;
    ...
    /**
     * \brief Do initial XML write (nodes, links, etc.). Scheduled at t=0 so AddHiddenNode/AddCustomLink can be called first.
     */
    void DoInitialWrite();
    /**
